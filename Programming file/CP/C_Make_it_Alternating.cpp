...
vector<ll> storeFact(200005);
void factorialPreCount()
{
    storeFact[0] = 1;
    ll mult = 1;
...
void solve()
{
    ...
    for (int i = 0; i < n - 1; i++)
    {
        ...
        {
            // cnt --> count of the same number that i can delete --> 1 1 1 
                                                                   // ^ ^ ^ can delete this 1 or this 1 or this 1 --> 3 times
            ans = (ans * cnt) % MOD;
            cnt = 1;
        }
    }
    ...
    ans = (ans * storeFact[freq]) % MOD; // number of ways i can choose a number from the list to delete --> storeFact[freq]
