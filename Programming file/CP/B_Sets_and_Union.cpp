...
void solve()
{
    ...
    for (int i = 0; i < 55; i++)
    {
        if (real & (1LL << i)) // checking if (1LL << i) occurs in any given set
        {
            ll num = 0;
            for (int j = 0; j < n; j++)
            {
                if (a[j] & (1LL << i)) // as we don't want to include (1LL << i) because in that case we can obtain a set
                                       // that has at least 1 item less than the real set and hence we check this condition.
                {
                    continue;
                }
                num |= a[j];
