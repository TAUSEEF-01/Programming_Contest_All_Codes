...
ll calc(ll n, ll turn)
{
    ...
    {
        if (dp[n + 1][turn - 1] == -1)
            result = calc(n + 1, turn - 1) % mod;

        else
            result = dp[n + 1][turn - 1];
    }
    else
    {
        if (dp[1][turn - 1] == -1 && dp[0][turn - 1] == -1)
            result = (calc(1, turn - 1) % mod + calc(0, turn - 1) % mod) % mod;

        else if (dp[1][turn - 1] != -1 && dp[0][turn - 1] == -1)
            result = (dp[1][turn - 1] % mod + calc(0, turn - 1) % mod) % mod;

        else if (dp[1][turn - 1] == -1 && dp[0][turn - 1] != -1)
            result = (calc(1, turn - 1) % mod + dp[0][turn - 1] % mod) % mod;

        else
            result = (dp[1][turn - 1] % mod + dp[0][turn - 1] % mod) % mod;
    }

    // cout << result << ' ';
    return dp[n][turn] = result % mod;
}
*/
