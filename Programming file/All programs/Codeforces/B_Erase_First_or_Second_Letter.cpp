...
void solve()
{
    ...
    vector<ll> ans(n, 0);
    vector<int> cnt(300, n); // keep track of the index of the number where it was last used --> initializing the value  (vvi)

    cnt[a[n - 1]] = n - 1;
    ans[n - 1] = 1;
    for (int i = n - 2; i >= 0; i--)
    {
        ans[i] = ans[i + 1] + (cnt[a[i]] - i); // keep track of the index of the number where it was last used (vvi)
        cnt[a[i]] = i;
    }
