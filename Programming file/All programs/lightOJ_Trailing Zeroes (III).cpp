...
ll calculateTraillingZeros(ll number) 
{
    ll traillingZeros = 0;
...
int main() 
{
    ...
    while(t--)
    {
        ll requiredTraillingZeros;
        cin>>requiredTraillingZeros;

        ll low = 1, high = LIMIT, answer = -1;
        while (low <= high) 
        {
            ll mid = (low + high) / 2;
            ll traillingZeros = calculateTraillingZeros(mid);
